...
function validateRecords(records) {
  ...
  const allowedCategories = new Set(['academic', 'tech']);
  const allowedFormats = new Set(['svg', 'png']);

  records.forEach((record, index) => {
    const rowNumber = index + 2;
    if (!Number.isInteger(record.id) || record.id <= 0) {
      throw new Error(`Invalid id at CSV row ${rowNumber}: expected positive integer.`);
    }
    if (seenIds.has(record.id)) {
      throw new Error(`Duplicate id ${record.id} detected at CSV row ${rowNumber}.`);
    }
    seenIds.add(record.id);

    const slug = record.slug || '';
    if (!slug) {
      throw new Error(`Missing slug at CSV row ${rowNumber}.`);
    }
    if (!/^[a-z0-9-]+$/.test(slug)) {
      throw new Error(`Invalid slug "${slug}" at CSV row ${rowNumber}. Use lowercase letters, digits, and hyphen.`);
    }
    if (seenSlugs.has(slug)) {
      throw new Error(`Duplicate slug "${slug}" detected at CSV row ${rowNumber}.`);
    }
    seenSlugs.add(slug);

    if (!allowedCategories.has(record.category)) {
      throw new Error(`Invalid category "${record.category}" at CSV row ${rowNumber}. Allowed: academic, tech.`);
    }

    if (!allowedFormats.has(record.logo_format)) {
      throw new Error(`Unsupported logo_format "${record.logo_format}" at CSV row ${rowNumber}. Allowed: svg, png.`);
    }
    ...
    try {
      ...
      throw new Error(`Invalid URL for slug "${slug}" at CSV row ${rowNumber}.`);
    }

    if (!record.official_name) {
      throw new Error(`Missing official_name for slug "${slug}" at CSV row ${rowNumber}.`);
    }

    ['alt_ru', 'alt_en', 'title_ru', 'title_en', 'meta_ru', 'meta_en'].forEach((field) => {
      if (!record[field]) {
        throw new Error(`Missing ${field} for slug "${slug}" at CSV row ${rowNumber}.`);
      }
    });
